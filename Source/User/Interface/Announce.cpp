...
void Announce::AddMsg(std::string msg, TCODColor color, const Coordinate& coordinate)
{
	...
	if (!messageQueue.empty() && messageQueue.back()->msg == msg &&
		messageQueue.back()->target == coordinate)
	{
	...
	if (!messageQueue.empty())
	{
		...
		if (timer > 0.5 * UPDATES_PER_SECOND)
		{
			if (messageQueue.size() > (unsigned int)ANNOUNCE_HEIGHT)
			{
				...
				length = 0;
				for (std::deque<AnnounceMessage*>::iterator msgi = messageQueue.begin();
					 msgi != messageQueue.end(); ++msgi)
				{
...
MenuResult Announce::Update(int x, int y, bool clicked)
{
	...
	{
		if (clicked && y > (signed int)top && (y - top - 1) >= 0 &&
			(y - top - 1) < messageQueue.size())
		{
...
{
	int count = 0;
	for (std::deque<AnnounceMessage*>::reverse_iterator ani = messageQueue.rbegin();
		 ani != messageQueue.rend(); ++ani)
	{
		if (count++ >= from)
		{
	...
	for (std::deque<AnnounceMessage*>::reverse_iterator ani = history.rbegin();
		 ani != history.rend(); ++ani)
	{
		if (count++ >= from)
		{
			console->print(pos.getX(), pos.getY() + count - from, (*ani)->ToString().c_str());
